...
print(t.children)
print(t.children[0]) # prints subtree associated with the first child node
print(t.children[1]) # prints subtree associated with the second child node
...
# since its the root node it doesnt have a parent, so it returns the default value (1.0)
print(t.dist)

# .is_leaf() returns True when TreeNode object t is a leaf, False when it is an internal node
print( t.is_leaf() )

# .get_tree_root returns the root of tree that the TreeNode object is in
# in this case it returns the same node, since the considered TreeNode object is already the root
# the print statement then prints the tree again in ascii characters
print( t.get_tree_root() )

# .get_tree_root() applied on the TreeNode t.children[0] returns the root of entire tree t
